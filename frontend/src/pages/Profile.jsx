...
export default function Profile() {
  ...
  const onUpload = async (event) => {
    ...
    try {
      ...
      setError(err.response?.data?.errorMessage || 'Failed to upload profile photo.')
    } finally {
      setUploading(false)
      event.target.value = ''
    }
        ...
        <div style={avatarRow}>
          <div style={avatarWrap}>
            {profile?.profilePicture ? (
              <img src={toImageUrl(profile.profilePicture)} alt="Profile" style={avatarImage} />
            ) : (
...
const avatarWrap = { width: 88, height: 88, borderRadius: '50%', overflow: 'hidden' }
...
const fieldLabel = { fontSize: '0.78rem', color: 'var(--muted)', marginBottom: '0.25rem' }
const fieldValue = { fontSize: '0.95rem', color: 'var(--ink)', fontWeight: 600 }
